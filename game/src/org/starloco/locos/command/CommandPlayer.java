...
public class CommandPlayer {
    ...
    public static boolean analyse(Player player, String msg) {
        ...
        if (msg.charAt(0) == '.') {
            ...
            if(command(msg, "help")) {
                ...
                // Commande inconnue : si GM, tenter une commande admin (ex: .LEVEL 200)
                if (dispatchAdminChat(player, msg)) {
                    return true;
                }
    ...
    private static boolean isGameMaster(Player player) {
        return player.getGroup() != null && !player.getGroup().isPlayer();
    }

    /**
     * Route les commandes GM via le chat (réponses en chat, sans console Flash).
     * Préfixe recommandé : {@code .a HELP} ou {@code .gm LEVEL 200}.
     */
    private static boolean dispatchAdminChat(Player player, String msg) {
        ...
        if (body.isEmpty()) {
            player.sendMessage("Commandes GM via le chat : <b>.a HELP</b> — ex: <b>.a LEVEL 200</b>, <b>.a KAMAS 1000000</b>");
            return true;
        }

        new ChatCommandAdmin(player).apply("BA" + body);
        return true;
    }

    /** AdminUser qui affiche les réponses dans le chat (cs) au lieu de la console BAT. */
    private static final class ChatCommandAdmin extends CommandAdmin {
        private ChatCommandAdmin(Player player) {
            super(player);
        }
